...
use crate::vm::definitions::{
    Instruction, JumpCondition, Source, UnaryOperation, COMPARE_REGISTER, RESULT_REGISTER,
};
...
fn translate_subtree(tree: &Node, stack_ptr: u8, next_label: &mut u16) -> Vec<Instruction> {
    match tree {
        ...
        Node::Val(src) => {
            // r[stack_ptr] = src
            vec![Instruction::Binary(stack_ptr, *src, Set)]
        }
        Node::Unary(op, child) => {
        ...
        Node::Binary(op, left, right) => {
            ...
            instr.push(Instruction::Binary(
                stack_ptr,
                Source::Register(stack_ptr + 1),
                *op,
            ));
        ...
        Node::Store(reg, child) => {
            // stores result of child in reg
        ...
        Node::While(condition, body) => {
            ...
            *next_label += 2;
            ...
            instr.push(Instruction::Jump(
                label_after,
                JumpCondition::Zero(stack_ptr + 1),
            ));
        ...
        Node::Compare(left, right) => {
            ...
            instr.push(Instruction::Compare(
                stack_ptr,
                Source::Register(stack_ptr + 1),
            ));
            ...
            instr.push(Instruction::Binary(
                stack_ptr,
                Source::Register(COMPARE_REGISTER),
                Set,
            ));
