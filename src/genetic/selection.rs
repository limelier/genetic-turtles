...
impl Generation {
    ...
    pub fn select_weighted_by_fitness(&self) -> (Vec<usize>, Vec<Parents>) {
        let mut best_score = self.population[self.best_index.unwrap()]
            .result
            .unwrap()
            .score;
        let worst_score = self.population[self.worst_index.unwrap()]
            .result
            .unwrap()
            .score;
        ...
        let fits = if best_score.is_finite() {
            ...
            for i in 0..POPULATION_SIZE {
                ...
                fits.push(if score.is_finite() {
                    ((score - worst_score) / (best_score - worst_score) + 1.0)
                        .powf(SELECTION_PRESSURE)
                } else {
                    0.0
                });
                1;
    ...
    fn tournament(&self) -> usize {
        ...
        members.sort_unstable_by(|&a, &b| -> Ordering {
            self.population[a]
                .result
                .partial_cmp(&self.population[b].result)
                .unwrap()
        });
