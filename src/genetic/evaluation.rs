    ...
    pub fn evaluate(&mut self, target: &BlockSpace) {
        for index in 0..self.population.len() {
            ...
            if self.best_index == None
                || result > self.population[self.best_index.unwrap()].result.unwrap()
            {
            ...
            if result.score.is_finite()
                && (self.worst_index == None
                    || result < self.population[self.worst_index.unwrap()].result.unwrap())
            {
...
fn evaluate(individual: &mut Individual, target: &BlockSpace) {
    ...
    individual.result = Some(match result {
        Ok(()) => {
            ...
            let score = dice_index * (DEPTH_SOFTENER + MAX_DEPTH as f64 - depth as f64)
...
mod test {
    ...
    */
    ...
    fn test_eval_perfect() {
        ...
        let tree = Node::Then(
            Box::from(Node::Store(0, Box::from(Node::Val(Source::Value(8))))),
            Box::from(Node::While(
